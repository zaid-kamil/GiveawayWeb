...
def viewGiveawayDetail(request, id):
    ...
    comments = Comment.objects.filter(gid=id)
    form = CommentForm(request.POST or None)
    context = {}
    if request.user.is_authenticated:
        entry, created = GiveawayEntry.objects.get_or_create(user=request.user.id, giveaway=id)
        context['entry'] = entry
        print(entry)
    ...
    if form.is_valid():
        ...
        # message success
        messages.success(request, "Successfully Created")
        Giveaway.objects.filter(id=id).update(comments=item.comments + 1)
        return HttpResponseRedirect(request.path)
    context["item"] = item
...
def update_entry(request, id, s):
    ...
    if entry:
        if share == 'facebook':
            GiveawayEntry.objects.filter(user=user_id, giveaway=id).update(
                facebook_share_count=entry.facebook_share_count + 1, total_points=entry.total_points + 1)
        if share == 'twitter':
            GiveawayEntry.objects.filter(user=user_id, giveaway=id).update(
                twitter_share_count=entry.twitter_share_count + 1,
                total_points=entry.total_points + 1)
        if share == 'google':
            GiveawayEntry.objects.filter(user=user_id, giveaway=id).update(
                google_plus_share_count=entry.google_plus_share_count + 1,
                total_points=entry.total_points + 1)
        if share == 'stumble':
            GiveawayEntry.objects.filter(user=user_id, giveaway=id).update(
                stumble_share_count=entry.stumble_share_count + 1,
                total_points=entry.total_points + 1)
        if share == 'linked':
            GiveawayEntry.objects.filter(user=user_id, giveaway=id).update(
                linked_share_count=entry.linked_share_count + 1,
                total_points=entry.total_points + 1)
        Giveaway.objects.filter(id=id).update(entries=Giveaway.objects.get(id=id).entries + 1)
        response = {"facebook": entry.facebook_share_count,
                    ...
                    "total": Giveaway.objects.get(id=id).entries,
                    "points": GiveawayEntry.objects.get(user=user_id, giveaway=id).total_points
                    }
